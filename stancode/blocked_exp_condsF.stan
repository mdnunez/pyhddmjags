...
functions { 
   ...
   */ 
   real diffusionmix_lpdf(real Y, real boundary, 
    ...
    if (Y >= 0) {
        return log_mix( theta, wiener_lpdf( fabs(Y) | boundary, ndt, bias, drift ), uniform_lpdf(fabs(Y) | 0 , seccutoff) );
    } else {
        return log_mix( theta, wiener_lpdf( fabs(Y) | boundary, ndt, 1-bias, -drift ), uniform_lpdf(fabs(Y) | 0 , seccutoff) );
    }
