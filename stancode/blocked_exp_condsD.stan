   ...
   */ 
   real diffusionmix_lpdf(real Y, real boundary, 
    ...
    if (fabs(Y) < ndt) {
        ...
        if (Y >= 0) {
            return log_mix( theta, wiener_lpdf( (fabs(Y)-ndt) | boundary, 0, bias, drift ), uniform_lpdf(fabs(Y) | 0 , seccutoff) );
        } else {
            return log_mix( theta, wiener_lpdf( (fabs(Y)-ndt) | boundary, 0, 1-bias, -drift ), uniform_lpdf(fabs(Y) | 0 , seccutoff) );
        }
    }
