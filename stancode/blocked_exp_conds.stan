...
parameters {
    ...
    matrix<lower=.1, upper=1>[nparts,nconds] ter; // Non-decision time
    matrix<lower=0, upper=2>[nparts,nconds] alpha; // Boundary parameter (speed-accuracy tradeoff)
}
...
model {
    ...
    for (p in 1:nparts) {
        ...
        for (c in 1:nconds) {
            ...
            // Non-decision time
            ter[p,c] ~ normal(terpart[p], tersdcond) T[.1, 1];

            // Boundary parameter (speed-accuracy tradeoff)
            alpha[p,c] ~ normal(alphapart[p], alphasdcond) T[0, 2];
